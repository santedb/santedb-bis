...
>
	...
	<xsl:template match="//html:table">
		...
		<s:Workbook>
			<o:DocumentProperties>
				<o:LastAuthor>SanteDB CDR Server</o:LastAuthor>
				<o:Created>
			...
			<s:Worksheet ss:Name="santedb-output">
				<x:WorksheetOptions>
					...
					<x:FrozenNoSplit />
					<x:SplitHorizontal>
						<xsl:value-of select="count(html:thead/html:tr)" />
					</x:SplitHorizontal>
	...
	<xsl:template match="html:th">
		<s:Cell ss:StyleID="Header">
			...
			<xsl:if test="@_offset &gt; 0">
				<xsl:attribute name="ss:Index">
					<xsl:value-of select="@_offset + count(preceding-sibling::html:th) + 1" />
				</xsl:attribute>
			</xsl:if>
			<s:Data ss:Type="String">
				<xsl:value-of select="." />
